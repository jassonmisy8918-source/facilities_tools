...
const realtimeData = ref([
  { station: '和平路雨量站', value: 3.2, cumulative: 28.5, level: '小雨', status: 'normal', time: '14:30:05' },
  { station: '丰台区雨量站', value: 8.5, cumulative: 45.2, level: '大雨', status: 'warning', time: '14:30:02' },
  { station: '朝阳区雨量站', value: 1.8, cumulative: 12.6, level: '小雨', status: 'normal', time: '14:29:58' },
  { station: '海淀区雨量站', value: 0.4, cumulative: 3.8, level: '小雨', status: 'normal', time: '14:30:01' },
  { station: '通州区雨量站', value: 12.6, cumulative: 68.4, level: '大雨', status: 'danger', time: '14:30:03' },
  { station: '大兴区雨量站', value: 5.3, cumulative: 32.1, level: '中雨', status: 'warning', time: '14:29:59' },
])

// 历史数据
const historyData = ref([
  { date: '2024-03-15 14:00', station: '和平路雨量站', value: 3.2, cumulative: 28.5, level: '小雨', duration: '6h' },
  { date: '2024-03-15 13:00', station: '丰台区雨量站', value: 8.5, cumulative: 45.2, level: '大雨', duration: '8h' },
  { date: '2024-03-14 20:00', station: '通州区雨量站', value: 12.6, cumulative: 68.4, level: '大雨', duration: '10h' },
  { date: '2024-03-14 16:00', station: '朝阳区雨量站', value: 2.1, cumulative: 15.8, level: '小雨', duration: '4h' },
  { date: '2024-03-13 22:00', station: '大兴区雨量站', value: 5.6, cumulative: 32.1, level: '中雨', duration: '7h' },
  { date: '2024-03-12 08:00', station: '海淀区雨量站', value: 0.8, cumulative: 5.2, level: '小雨', duration: '3h' },
])
  ...
  <div class="space-y-4">
    ...
    <div class="bg-card border border-themed rounded-xl shadow-themed p-4">
      ...
      <div class="flex items-center gap-3 flex-wrap">
        <select v-model="selectedStation" class="bg-input border border-themed rounded-lg px-3 py-1.5 text-xs text-default focus:outline-none focus:border-primary">
          <option v-for="s in stations" :key="s" :value="s">{{ s }}</option>
        </select>
        <div class="flex items-center gap-1.5 text-xs">
          <input v-model="startDate" type="date" class="bg-input border border-themed rounded-lg px-2 py-1.5 text-xs text-default focus:outline-none focus:border-primary" />
          <span class="text-dim">至</span>
          <input v-model="endDate" type="date" class="bg-input border border-themed rounded-lg px-2 py-1.5 text-xs text-default focus:outline-none focus:border-primary" />
        </div>
        <button class="flex items-center gap-1.5 px-4 py-1.5 bg-primary text-white rounded-lg text-xs font-medium hover:bg-primary-light transition-colors cursor-pointer">
          <Search class="w-3.5 h-3.5" /> 查询
        </button>
        <button class="p-1.5 rounded hover:bg-hover-themed transition-colors cursor-pointer"><RefreshCw class="w-3.5 h-3.5 text-dim" /></button>
      </div>
    </div>

    <!-- 实时数据 -->
    <div v-if="activeFunc === 'realtime'" class="bg-card border border-themed rounded-xl shadow-themed overflow-hidden">
      <table class="w-full text-xs"><thead><tr class="border-b border-themed bg-surface/50">
        ...
        <tr v-for="d in realtimeData" :key="d.station" class="border-b border-themed/30 hover:bg-hover-themed transition-colors">
          ...
          <td class="text-center px-4 py-2.5"><span class="text-[10px] px-2 py-0.5 rounded font-medium" :class="[getLevelColor(d.level), getLevelBg(d.level)]">{{ d.level }}</span></td>
          <td class="text-center px-4 py-2.5"><span :class="getStatusColor(d.status)">{{ d.status === 'normal' ? '正常' : d.status === 'warning' ? '预警' : '报警' }}</span></td>
    ...
    <div v-if="activeFunc === 'history'" class="bg-card border border-themed rounded-xl shadow-themed overflow-hidden">
      <table class="w-full text-xs"><thead><tr class="border-b border-themed bg-surface/50">
        ...
        <tr v-for="d in historyData" :key="d.date + d.station" class="border-b border-themed/30 hover:bg-hover-themed transition-colors">
